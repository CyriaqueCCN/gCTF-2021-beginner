...
We are explicitly allowed to bruteforce so we probably need to test all permutations of the digits 35780 on the url https://old-lock-web.2021.ctfcompetition.com/

We write a little script to bruteforce it using itertools and requests
If we don't see the error message in the result, we found the flag so we print it and exit

Flag is `CTF{IThinkWeNeedToReplaceTheKeypad}`
